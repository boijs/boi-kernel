...
const DEFAULT_CONFIG = require('../constants').deploy;
...
function uploadFiles(ftp, srcDir, targetPath) {
  ...
  glob(_srcFiles, (err, files) => {
    ...
    if (!files || files.length === 0) {
      Log.error('No files found in ' + src);
      ftp.destroy();
      process.exit();
    }
...
module.exports = {
  ...
  config: function(options) {
    ...
    this.options = mergeMap(DEFAULT_CONFIG, _options);
    let _cdn = {
      default: {
        domain: this.options.default.cdn.domain,
        path: this.options.default.cdn.path
      },
      image: {
        domain: this.options.image.cdn.domain,
        path: this.options.image.cdn.path
      }
    };
    // cdn信息以全局变量的方式传递
