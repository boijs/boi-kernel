    ...
    loaders.length === 0 && loaders.push((() => {
      ...
      if (options.base64) {
        if (process.env.BOI_ENV === ENV.development) {
          ...
          _name = options.useHash ? '[name].[hash:8].[ext]' : '[name].[ext]';
        }
        _loaders.push('url-loader?limit=' + options.base64Limit + '&name=' + _name);
      } else {
        if (process.env.BOI_ENV === ENV.development) {
          _name = '[name].[ext]';
        } else {
          let _cdn = global.boi_deploy_cdn && global.boi_deploy_cdn.image;
          _name = (_cdn && _cdn.domain ? '' : path.posix.join('/', options.destDir, '/')) +
            (options.useHash ? '[name].[hash:8].[ext]' : '[name].[ext]')
        }
        _loaders.push('file-loader?name=' + _name);
      }
      _loaders.push('image?{bypassOnDebug:true, progressive:true,optimizationLevel:3,pngquant:{quality:"65-80",speed:4}}');
      return {
        test: REG_EXTTYPE,
        loaders: _loaders
      };
