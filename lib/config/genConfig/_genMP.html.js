...
module.exports = function(config) {
  ...
  if (_config.webpackConfig && _config.webpackConfig.plugins && _config.webpackConfig.plugins.length !== 0) {
    ...
    let __plugins = ((options) => {
      let __htmlPlugins = [];
      let _srcTpl = path.posix.resolve(process.cwd(), config.basic.localPath.src, options.srcDir);
      ...
      __htmlPlugins.push(new HtmlWebpackReplaceurlPlugin({
        js: {
          ...
          useHash: _env === ENV.production && config.js.useHash || false
        },
        style: {
          mainFilePrefix: config.style.files ? '' : config.style.mainFilePrefix,
          useHash: _env === ENV.production && config.style.useHash || false
        },
      ...
      if (_config.favicon) {
        __htmlPlugins.push(new FaviconsWebpackPlugin({
          logo: path.posix.join(process.cwd(), _config.favicon),
          inject: true,
