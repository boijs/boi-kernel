...
module.exports = function(config) {
  ...
  (() => {
     ...
     */
    process.env.BOI_ENV!==ENV.development&&CONFIG_STYLE.lint && plugins.push(new StyleLintPlugin({
      configFile: CONFIG_STYLE.lintConfigFile ? path.posix.resolve(process.cwd(), CONFIG_STYLE.lintConfigFile) : require
        .resolve('../../../../boi-aux-rule-stylelint'),
    ...
    loaders.length === 0 && (() => {
      ...
      let _imageCdn = null;
      if (process.env.BOI_ENV === ENV.production) {
        if (config.image.cdn) {
          _imageCdn = 'http://' + config.image.cdn;
        } else if (config.basic.cdn && config.basic.cdn.server && config.basic.cdn.path) {
          _imageCdn = 'http://' + path.posix.join(config.basic.cdn.server, config.basic.cdn
            .path)
        }
      }
