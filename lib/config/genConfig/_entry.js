...
module.exports = function(config) {
    ...
    // js文件目录绝对路径
    let _jsDir = path.posix.join(process.cwd(), config.basic.localPath.src, config.js.srcDir);
    ...
    if (_configJs.files && _configJs.files.vendor && _configJs.files.vendor.length !== 0) {
        ...
        _plugins.push(new webpack.optimize.CommonsChunkPlugin({
            name: 'vendor',
            filename: path.posix.join(process.cwd(),congifg.js.destDir,'/vendor/vendor.js'),
            // @important chunks字段必填,否则会将style文件打包在一起
            chunks: _entry['vendor']
        }));
    } else if (_configJs.mutiEntriesVendor && _.keys(_entry).length > 1) {
        ...
        _plugins.push(new webpack.optimize.CommonsChunkPlugin({
            name: 'vendor',
            filename: path.posix.join(config.js.destDir,'/vendor/vendor.js'),
            // @important chunks字段必填,否则会将style文件打包在一起
            chunks: []
        }));
    }
