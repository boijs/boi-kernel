...
const EXT_DEPS_MAP = {
  ...
  less: ['css-loader', 'less', 'less-loader', 'postcss-less',
    'postcss-loader'],
...
module.exports = function (config) {
  ...
  if (CONFIG_STYLE.webpackConfig) {
    ...
    let getLoaders = ((autoprefix, spritesInfo) => {
      let baseLoaders = [];
      autoprefix ? baseLoaders.push('css-loader') : baseLoaders.push(
      ...
      if (spritesInfo) {
        ...
        baseLoaders.push({
          ...
          options: {
            plugins: require('postcss-sprites')(postcssSpritesOpts)
          }
        });
      }
    ...
    rules = [{
      ...
      use: `file-loader?name=${Path.basename(config.image.output)}/[name].[ext]`
      }, {
      test: REG_EXT,
      use: getLoaders[EXT]
      }];
    ...
    // 收集依赖模块
    dependencies = EXT === 'css' ? [] : EXT_DEPS_MAP[EXt].filter(loader => {
