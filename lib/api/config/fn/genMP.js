...
module.exports = function(config, env) {
    ...
    _plugins.push(new WebpackMd5Hash());
    ...
    _loader_image = (config.image.webpackConfig&&config.image.webpackConfig.loader) || ((opts) => {
        ...
        if (opts.base64) {
            __loaders.push('url-loader?limit=' + (opts.base64Limit | '10000') +
                '&name=' + opts.destDir + (opts.useHash ? '/[name].[hash:8].[ext]' : '/[name].[ext]'));
        } else {
            __loaders.push('file-loader?name=' + (opts.cdn ? '' : (opts.destDir || 'image')) +
                (opts.useHash ? '/[name].[hash:8].[ext]' : '/[name].[ext]'));
        }
    ...
    let _mp_js = fn_genMPJs(config,env);
    if (_mp_js) {
        if (_mp_js.preloader) {
