...
module.exports = function(config) {
    ...
    if (_configJs.files && _configJs.files.main && _.isPureObject(_configJs.files.main)) {
        for (let filename in _configJs.files.main) {
            if (REG_MAIN_FULL.test(_configJs.files.main[filename])) {
                _entry[filename] = path.join(_jsDir, _configJs.files.main[filename]);
            } else {
                _entry[filename] = path.join(_jsDir, _configJs.files.main[filename] + '.' + _extType);
            }
        }
    } else {
    ...
    } else if (_configJs.mutiEntriesVendor && _.getKeysSum(_entry) > 1) {
