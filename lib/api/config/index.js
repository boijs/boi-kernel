...
api.runBuild = function(env) {
    ...
    let _config = genConfig(boiConfig, _env);
    let _dependencies = _config.dependencies;
    let _cache = boi.cache.get(PATH_HASH);
    if(!_cache){
        boi.cache.set(PATH_HASH,{
            boi: _.assign({},boiConfig),
            // wp: _.assign({},_config.wp),
            deps: _config.dependencies
        });
    }

    if(!boi.cache.get(PATH_HASH).depInstalled){
        let __modules = '';
        _dependencies.forEach(function(module){
            try{
                require.resolve(path.join(process.cwd(),'node_modules',module))||require.resolve(module);
            }catch(e){
                __modules = __modules + module + ' ';
            }
        });
        if(__modules!==''){
            exec('npm isntall '+__modules+' --save-dev')
        }
        boi.cache.update(PATH_HASH,{depInstalled:true});
    }

    webpack(_config.wp).run(function(err, stat) {
        if (err) {
            throw err;
        }
...
api.genDevConfig = function() {
    let _cache = boi.cache.get(PATH_HASH);
    if(_cache){
        return genConfig(_cache.boi,'dev').wp;
    }else{
        return genConfig(boiConfig, 'dev').wp;
    }
};
/**
 * 获取boiConfig
 ...
 */
module.exports = {
    // webpackConfig: cache.get(PATH_HASH).wp,
    api: api
};
