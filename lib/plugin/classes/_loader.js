...
module.exports = function(pattern, options) {
    ...
    if (pattern && options && _.isString(pattern) && _.isPlainObject(options)) {
        this.pattern = pattern;
        this.options = options;
    } else if (pattern && _.isPlainObject(pattern)) {
        // extend - 如果已存在同名配置，则在原有配置基础上进行增量更新
        // override - 不论是否已存在同名配置，插件的配置项完全覆盖原有配置
        this.pattern = 'extend';
        this.options = pattern;
    } else if (!pattern && _.isNull(pattern) && _.isPlainObject(options)) {
        this.pattern = 'extend';
        this.options = options;
    } else {
