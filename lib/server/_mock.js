...
let excute = function(pattern, response) {
  ...
  REG_URL_PARAMTERS.test(urlPattern) && (((pattern) => {
    api = pattern.replace(REG_URL_PARAMTERS, '');
    let _item = REG_URL_QUERY_ITEM.exec(pattern);
    while (_item) {
      queries.push(_item[0].replace('$', ''));
      _item = REG_URL_QUERY_ITEM.exec(pattern);
    }
  })(urlPattern));
...
let route_post = function(api, response) {
  let res_ok = response.ok || {
    code: 100,
    msg: '请求成功'
  };
