...
let middleware_redirurl = function(req, res, next) {
  ...
  if (req.url == '/') {
    ...
    if (files.length === 1) {
      req.url = path.posix.join('/', boiConfig.html.srcDir, path.basename(files[0]));
    }
  } else if (/\.html$/.test(req.url) && /\w+/.test(boiConfig.html.srcDir)) {
    let dirname = /\w+/.exec(boiConfig.html.srcDir)[0];
