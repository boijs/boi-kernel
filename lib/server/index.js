...
let middleware_redirurl = function(req, res, next) {
  ...
  if (req.url == '/') {
    ...
    if (files.length === 1) {
      req.url = path.posix.join('/', boiConfig.html.source, path.basename(files[0]).replace(path.extname(files[0]),'.html'));
    }
...
server.run = function() {
  ...
  _devMiddleware.waitUntilValid(function () {
    console.log(chalk.cyan.bold('Server is listening at localhost:' + configuration.port + '\n'));
  });
  // html文档更新触发浏览器自动刷新
