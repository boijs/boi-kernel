...
let middleware_redirurl = function(req, res, next) {
    ...
    if (req.url === '/') {
        let htmlDir = path.posix.resolve(process.cwd(), boiConfig.basic.localPath.src, boiConfig.html.srcDir);
...
server.run = function() {
    ...
    if (!webpackConfig) {
        webpackConfig = api_config.genConfig();
    }
    ...
    if (boiConfig.basic.localPath.thirdparty) {
        let _staticPath = path.posix.resolve(process.cwd(), boiConfig.basic.localPath.thirdparty);
